...
def parse_args():
    ...
    parser.add_argument('--finetune_idx',
                        type=float,
                        default=0,
                        help='finetuned index')
...
def train(device, train_loader, model, criterion, optimizer, scheduler, epoch, args):
    ...
    for one_data in train_loader:
        ...
        # # use inference time weight decay
        # if not args.pruned_finetune and hasattr(model.module.backbone, 'get_custom_l2'):
        #     loss += model.backbone.get_custom_l2(weight_decay=args.weight_decay)
...
def main():
    ...
    if args.pruned_finetune:
        ...
        # # load last finetune model
        # last_idx = args.finetune_idx - 1
        # if last_idx > 0:
        #     msg = model.load_state_dict(torch.load(os.path.join(args.checkpoints, f'pruned_last_{last_idx}.pth')), strict=False)
        #     print(msg)
    ...
    for key, value in model.named_parameters():
        if not value.requires_grad:
            continue
        # rbr weight decay is added in loss
