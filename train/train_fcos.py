...
from public.dataset.cocodataset import COCODataPrefetcher, Collater
from public.loss import FCOSLoss
from public.decoder import FCOSDecoder
...
def evaluate_coco(val_dataset, model, decoder, args):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(val_loader)):
            images, scales = torch.tensor(data['img'], dtype=torch.float), torch.tensor(data['scale'])
    ...
    json.dump(results,
              open('{}_{}_{}_{}_bbox_results.json'.format(val_dataset.set_name, Config.network,
                     Config.backbone, Config.version), 'w'),
              indent=4)

    # load results in COCO evaluation tool
    coco_true = val_dataset.coco
    coco_pred = coco_true.loadRes('{}_{}_{}_{}_bbox_results.json'.format(val_dataset.set_name, Config.network,
                     Config.backbone, Config.version))
...
def main():
    ...
    dist.init_process_group(backend='nccl', init_method='env://', world_size=gpus_num)
    ...
    # if Config.pretrained:
        # if args.version == 1 or args.version == 2 or Config.multi_task:
    ...
    #     # for p in model.clsregcnt_head_1.parameters():
    #     #         p.requires_grad = False
    ...
    criterion = FCOSLoss(**Config.loss_dict).to(device)
    decoder = FCOSDecoder(**Config.decoder_dict).to(device)
...
def train(device, train_loader, model, criterion, optimizer, scheduler, epoch, args):
    ...
    while images is not None:
        ...
        loss = cls_loss + 0.1*reg_loss + center_ness_loss
        if loss.item() == 0.0:
            optimizer.zero_grad()
            images, annotations = prefetcher.next()
            continue
...
#         loss.backward()
        ...
        losses.append(cls_loss.item()+reg_loss.item()+center_ness_loss.item())
