...
def parse_args():
    ...
    parser.add_argument('--sparse_rate',
                        type=float,
                        default=0,
                        help='scale aparse rate')
    parser.add_argument('--pruned_finetune',
                        action='store_true',
                        help='use pruned finetune mode')
...
def train(device, train_loader, model, criterion, optimizer, scheduler, epoch, args):
    ...
    for one_data in train_loader:
        ...
        # use inference time weight decay, only DDP adapted
        if not args.pruned_finetune and hasattr(model.module.backbone, 'get_custom_l2'):
            loss += model.module.backbone.get_custom_l2(weight_decay=args.weight_decay)
        ...
        # sparse loss
        if args.sparse_rate > 0:
            if isinstance(model, nn.parallel.DistributedDataParallel):
                model.module.update_mask(args.sparse_rate, 5e-4)
            else:
                model.update_mask(args.sparse_rate, 5e-4)
...
def evaluate_coco(val_dataset, model, decoder, args):
    ...
    per_image_testing_time = inference_time / len(val_dataset)
    logger.info(
        f"testing_time: {testing_time:.3f}, per_image_inference_time: {per_image_testing_time:.6f}"
    )
    ...
    json_name = '{}_{}_{}_{}_bbox_results.json'.format(
        val_dataset.set_name, Config.network, Config.backbone_type, Config.version
    )
...
def main():
    ...
    # load model
    is_pruning_model = 'pruning' in args.backbone_type
    assert is_pruning_model, 'backbone type must be pruning_model.'

    model = models.__dict__[args.network](**{
        "pretrained": args.pretrained and not args.pruned_finetune,
    ...
    # only prune backbone
    # model.backbone.fix_layer_last_mask()

    if args.pruned_finetune:
        pruned_filename = os.path.join(args.checkpoints, 'best.pth')
        if local_rank == 0:
            logger.info(f"load pruned model from {pruned_filename}")
        
        msg = model.load_state_dict(torch.load(pruned_filename), strict=False)
        if local_rank == 0:
            print(msg)

        model.prune(args.prune_ratio)  # TODO: pruned model
        if local_rank == 0:
            print(model)
    ...
    # Weight decay removal for repvgg and rmnet
    no_weight_decay_list = []
    for key, value in model.named_parameters():
        if not value.requires_grad:
            continue
        is_custwd = 'rbr_dense' in key or 'rbr_1x1' in key  # rbr weight decay is added in loss
        is_mask = False # 'mask' in key and 'backbone' in key  # weight decay of mask = 0
        # TODO: and m.kernel_size == (1, 1) and m.groups != 1 and m.weight.requires_grad:
        if is_custwd or is_mask:
            no_weight_decay_list.append(key)
            # print('set weight decay=0 for {}'.format(key))

    # for SGD optimizer
    # param_groups, param_groups_names = param_groups_lrd(model, lr=args.lr, weight_decay=0.0004, no_weight_decay_list=no_weight_decay_list)
    # if local_rank == 0:
    #     print('parameter groups: \n{}'.format(json.dumps(param_groups_names, indent=2)))
    # optimizer = torch.optim.SGD(param_groups, momentum=0.9)
    
    # for AdamW optimizer
    param_groups, param_groups_names = param_groups_lrd(model, lr=args.lr, weight_decay=args.weight_decay, no_weight_decay_list=no_weight_decay_list)
    if local_rank == 0:
        print('parameter groups: \n{}'.format(json.dumps(param_groups_names, indent=2)))
    ...
    # resume training
    resume_status = ((not args.pruned_finetune) and os.path.exists(args.resume)) \
        or (args.pruned_finetune and os.path.exists(os.path.join(args.checkpoints, 'pruned_last.pth')))
    if resume_status:
        if local_rank == 0:
            logger.info(f"start resuming model from {args.resume}")
        
        resume_filename = os.path.join(args.checkpoints, 'pruned_last.pth') if args.pruned_finetune else args.resume
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if local_rank == 0:
            save_filename = os.path.join(args.checkpoints, 'pruned_last.pth') if args.pruned_finetune else os.path.join(args.checkpoints, 'latest.pth')
            torch.save(
                {
                    'epoch': epoch,
                    'best_map': best_map,
            ...
            if local_rank == 0:
                ...
                if all_eval_result is not None:
                    ...
                    if all_eval_result[0] > best_map:
                        save_filename = os.path.join(args.checkpoints, 'pruned_best.pth') if args.pruned_finetune else os.path.join(args.checkpoints, 'best.pth')
