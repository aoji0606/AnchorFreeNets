...
#         neck_dict = dict(num_layers=3, out_channels=[96, 96, 64], depthwise=False)
    ...
    if "TTF" in neck_type:
        ...
        if backbone_type == "resnet18":
            neck_out_channles = 64
            neck_dict = dict(out_channels=[256, 128, 64], upsample=False, selayer=False, deformable=False, depthwise=False)
        elif backbone_type == "mobilenetv2":
            neck_out_channles = 24
            neck_dict = dict(out_channels=[96, 32, 24], upsample=False, selayer=False, deformable=False, depthwise=False)
