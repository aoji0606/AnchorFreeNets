...
class COCO(data.Dataset):
    def __init__(self, data_dir, split, split_ratio=1.0, img_size=512, num_classes=80, max_objs=100):
        super(COCO, self).__init__()
        self.num_classes = num_classes
        self.valid_ids = [i for i in range(1, num_classes+1)]
    ...
    def __getitem__(self, index):
        ...
        img -= self.mean
        img /= self.std
        img = np.ascontiguousarray(img.transpose(2, 0, 1))  # from [H, W, C] to [C, H, W]
        ...
        return {'image': torch.tensor(img,dtype=torch.float32),
                ...
                'w_h_': torch.tensor(w_h_,dtype=torch.float32), 
                'regs': torch.tensor(regs,dtype=torch.float32), 
                'inds': torch.tensor(inds,dtype=torch.int64), 
                'ind_masks': torch.tensor(ind_masks,dtype=torch.float32)}
...
if __name__ == '__main__':
    ...
    dataset = COCO(data_dir='/home/jovyan/data-vol-polefs-1/dataset/flags', split='train', split_ratio=1.0, img_size=512,num_classes=15, max_objs=100)
#     for d in dataset:
#         b1 = d
#     #   pass
