...
__all__ = [
    ...
    'wide_resnet50_2',
    'wide_resnet101_2',
    'densecl_resnet50_coco',
    'densecl_resnet50_imagenet'
]
...
model_urls = {
    ...
    'empty',
    'densecl_resnet50_coco':
    '{}/resnet/densecl_r50_coco_1600ep.pth'.format(
        pretrained_models_path),
    'densecl_resnet50_imagenet':
    '{}/resnet/densecl_r50_imagenet_200ep.pth'.format(
        pretrained_models_path)
}
    ...
    if pretrained:
        if "densecl" in arch:
            model.load_state_dict(
                torch.load(model_urls[arch], map_location=torch.device('cpu'))["state_dict"], strict=False)
        else:
            model.load_state_dict(
                torch.load(model_urls[arch], map_location=torch.device('cpu')), strict=True)
...
def densecl_resnet50_coco(pretrained=False, progress=True, **kwargs):
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('densecl_resnet50_coco', Bottleneck, [3, 4, 6, 3], pretrained, progress,
                   **kwargs)

def densecl_resnet50_imagenet(pretrained=False, progress=True, **kwargs):
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('densecl_resnet50_imagenet', Bottleneck, [3, 4, 6, 3], pretrained, progress,
                   **kwargs)
